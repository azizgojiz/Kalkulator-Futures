...
def position_selector(state_key):
    ...
    # Force an immediate rerun on change so the active color updates
    # right away instead of lagging one click behind.
    if clicked_long and st.session_state[state_key] != "long":
        st.session_state[state_key] = "long"
        st.rerun()
    if clicked_short and st.session_state[state_key] != "short":
